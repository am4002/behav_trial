#!/usr/bin/env python
'''
    framework for running behavioral N-back tests using MEG on Windows OS
...
from nback_test_win import vNback, aNback, withinInterleaved, betweenInterleaved, nPaired, nUnpaired, vDistractor, aDistractor
...
if __name__ == '__main__':
    ...
    for test in funcLis:
        ...
        if MEG==True:
            test(outWr, 2, wnd, tSq, 1.0,inMEG=True)
        else:
            test(outWr, 2, wnd, tSq, 1.0) #filled with the generic arguments for all our test functions, change the number in seqGen() to make the list longer/shorter
