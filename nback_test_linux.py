    ...
    for n, trial in enumerate(stimlist):
        ...
        if inMEG == True:
            if target[n]==1:
                ...
                print 'trigger 2 UP'
## TODO - don't wait here, just set the port high for the while trial and go back to zero at the end
        if inMEG==True:
...
## TODO - don't write to the response port - just read : comment out #pport_resp.Out32           
            #pport_resp.Out32(pport_resp_addr, 0) # first, clear the line
            this_resp = 0
            # now wait for duration specified
            print 'wait for response'
